...
def compute_cost_regularised_new(X, y, theta, alpha, l):
    ...
    def ret_total_error(X, y, theta):
        total_squared_error = 0.0
        m = y.shape[0]
        for i in range(m):
            hypothesis = calculate_hypothesis(X, theta, i)
            output = y[i]
            squared_error = (hypothesis - output)
            total_squared_error += squared_error
    ...
    def ret_total_regularised_error(theta):
        ...
        for i in range(1,len(theta)):
            if i == 0:
                total_regularised_error += (theta[i] - ((alpha/m) * (ret_total_error(X,y,theta) * X[0])))**2
            else:
                total_regularised_error += (theta[i]*(1 - alpha*(l/m)) - ((alpha)*(1/m) * (ret_total_error(X,y,theta) * X[i])))**2
            # total_regularised_error += theta[i]**2
        return total_regularised_error

    total_squared_error = ret_total_squared_error(X, y, theta)
    total_regularised_error = l*ret_total_regularised_error(theta)
    J = (total_squared_error + total_regularised_error)/(2*m)
